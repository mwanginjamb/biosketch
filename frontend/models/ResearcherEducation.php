<?php

namespace frontend\models;

use Yii;

/**
 * This is the model class for table "researcher_education".
 *
 * @property int $id
 * @property int $researcher_id
 * @property string $degree
 * @property string $institution_name
 * @property string|null $field_of_study
 * @property int|null $graduation_year
 * @property int|null $sort_order
 * @property int|null $created_at
 * @property int|null $updated_at
 * @property int|null $created_by
 * @property int|null $updated_by
 *
 * @property Researcher $researcher
 */
class ResearcherEducation extends \yii\db\ActiveRecord
{


    /**
     * {@inheritdoc}
     */
    public static function tableName()
    {
        return 'researcher_education';
    }

    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['field_of_study', 'graduation_year', 'created_at', 'updated_at', 'created_by', 'updated_by'], 'default', 'value' => null],
            [['sort_order'], 'default', 'value' => 0],
            [['researcher_id', 'degree', 'institution_name'], 'required'],
            [['researcher_id', 'graduation_year', 'sort_order', 'created_at', 'updated_at', 'created_by', 'updated_by'], 'integer'],
            [['degree'], 'string', 'max' => 100],
            [['institution_name', 'field_of_study'], 'string', 'max' => 255],
            [['researcher_id'], 'exist', 'skipOnError' => true, 'targetClass' => Researcher::class, 'targetAttribute' => ['researcher_id' => 'id']],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function attributeLabels()
    {
        return [
            'id' => Yii::t('app', 'ID'),
            'researcher_id' => Yii::t('app', 'Researcher ID'),
            'degree' => Yii::t('app', 'Degree'),
            'institution_name' => Yii::t('app', 'Institution Name'),
            'field_of_study' => Yii::t('app', 'Field Of Study'),
            'graduation_year' => Yii::t('app', 'Graduation Year'),
            'sort_order' => Yii::t('app', 'Sort Order'),
            'created_at' => Yii::t('app', 'Created At'),
            'updated_at' => Yii::t('app', 'Updated At'),
            'created_by' => Yii::t('app', 'Created By'),
            'updated_by' => Yii::t('app', 'Updated By'),
        ];
    }

    /**
     * Gets query for [[Researcher]].
     *
     * @return \yii\db\ActiveQuery|\frontend\models\query\ResearcherQuery
     */
    public function getResearcher()
    {
        return $this->hasOne(Researcher::class, ['id' => 'researcher_id']);
    }

    /**
     * {@inheritdoc}
     * @return \frontend\models\query\ResearcherEducationQuery the active query used by this AR class.
     */
    public static function find()
    {
        return new \frontend\models\query\ResearcherEducationQuery(get_called_class());
    }

}
